...
| `bot/handlers/settings_handler.py` | Удаление данных пользователя (GDPR) | |
...
**Хендлер:** `confirm_delete_data()`

**Что удаляется:**
1. Личные подписки (`UserChannelSubscription`)
2. Добавленные подписки (`SourceSubscription`)
3. Созданные темы (`GroupTopic`)
4. Группы владельца (`ManagedGroup` где `creator_id = user_id`)
5. Настройки (`UserPreferences`)
6. Аккаунт (`TelegramAccount`) — **полное удаление**

**Логика:**
- Проверка состояния `Settings.confirm_delete`
- Каскадное удаление всех связанных данных
- Очистка Redis кеша
- Отправка сообщения об успехе (без главного меню!)
